...
// 1) 2024 Rank, 2023 Rank, Institution Name, Location Code, Location, Size, Focus, Research Output, Status, Academic Reputation Score, Academic Reputation Rank,
// Employer Reputation Score, Employer Reputation Rank, Faculty and Student Ratio Score, Faculty and Student Ratio Rank, Citations per Faculty Score, Citations per Faculty Rank,
// International Faculty Score, International Faculty Rank, International Students Score, International Students Rank, International Research Network Score, International Research Network Rank,
// Employment Outcomes Score, Employment Outcomes Rank, Sustainability Score, Sustainability Rank, Overall Score
// 2)Every attribute are declare as string because some contains =/+ in the same column, some are double and integer and we didn't need to use that number to calculate anything.
//3)At first, I didn’t include a condition to replace null or missing data with empty strings (""), which caused errors because the program expected every field to have a value
//Another reason is that missing or extra commas in the CSV can cause the tokenizer to miss or misread fields, leading to exceptions when accessing tokens.
